...
import com.example.backend.filters.binding.UserAuthenticated;
...
public class AdminFilter implements ContainerRequestFilter, ContainerResponseFilter {
    ...
    public void filter(ContainerRequestContext requestContext) throws IOException {
        ...
        if (authorization == null || authorization.equals("")) {
            requestContext.abortWith(Response.status(Status.NOT_FOUND).entity(ResponseMessage.NO_AUTHORIZATION).build());
            return;
        }
        ...
        if (payload == null) {
            requestContext.abortWith(Response.status(Status.NOT_FOUND).entity(ResponseMessage.INVALID_AUTHORIZATION_TOKEN).build());
            return;
        }

        UserPayload userPayload = UserJwtPayloadService.getUser(payload);

        if (userPayload.getUserType() != UserType.ADMIN) {
            requestContext.abortWith(Response.status(Status.NOT_FOUND).entity(ResponseMessage.NOT_AUTHORIZED).build());
            return;
        }
